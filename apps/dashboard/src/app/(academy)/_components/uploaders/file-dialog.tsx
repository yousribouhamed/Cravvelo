  ...
  return (
    <Dialog>
      ...
      <DialogContent
        ...
        title="    قم بتحميل الصور الخاصة بك"
      >
        <div className="p-4">
          <div
            {...getRootProps()}
            className={cn(
              "group relative  my-8 grid h-48  w-full cursor-pointer  place-items-center rounded-lg border-2 border-dashed border-muted-foreground/25 px-5 py-2.5 text-center transition hover:bg-muted/25",
            ...
            {isUploading ? (
              ...
              <div className="grid place-items-center gap-2 text-muted-foreground sm:px-5">
                <UploadIcon
                  ...
                  aria-hidden="true"
                />
                <p className="text-base font-medium">أسقط الملف هنا</p>
              </div>
            ) : (
              <div className="grid place-items-center gap-1 sm:px-5">
                ...
                <p className="text-sm text-slate-500">
                  {formatBytes(maxSize)} يرجى تحميل الملف بحجم أقل من
                </p>
              </div>
            )}
          </div>
        ...
        <p className="text-center text-sm font-medium text-muted-foreground">
          يمكنك تحميل ما يصل إلى {maxFiles} {maxFiles === 1 ? "file" : "files"}
        </p>
        {files?.length ? (
        ...
        {files?.length ? (
          <Button
            ...
            variant="ghost"
            size="sm"
            className="mt-2.5 w-full text-red-500"
...
function FileCard({ i, file, files, setFiles }: FileCardProps) {
  ...
  return (
    <div className="relative flex items-center justify-between gap-2.5 p-4">
      ...
      <div className="flex items-center gap-2">
        {file.type.startsWith("image") && (
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="max-w-2xl" title="قص الصوره">
              <div className="mt-8 grid place-items-center space-y-5">
                <Cropper
                  ...
                  guides={true}
                />
                <div className="flex items-center justify-center space-x-2 gap-x-4 p-4">
                  <Button
                    aria-label="Crop image"
                    type="button"
                    size="sm"
                    variant="ghost"
                    className="h-8 space-x-2 flex items-center justify-between"
                    onClick={() => {
                      onCrop();
                      setIsOpen(false);
                    }}
