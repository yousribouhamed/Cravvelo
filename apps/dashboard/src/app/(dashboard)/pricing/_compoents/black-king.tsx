...
import { black_king_plan } from "@/src/constants/plans";
import Image from "next/image";

const BlackKing: FC = ({}) => {
  return (
    <div className="w-full h-fit mt-12 min-h-[400px] grid grid-cols-3">
      <div
        key={black_king_plan.plan_code}
      ...
      >
        <div className="w-full h-[100px] flex items-center justify-start gap-x-4 ">
          ...
          <h2 className="font-bold text-2xl text-start text-black">
            {black_king_plan.plan}
          </h2>
        </div>
        <div className="w-full h-[100px] flex items-center justify-center">
          <p className="text-gray-500 text-xl">
            <span className="text-4xl font-bold mx-3 text-black">
              DZD {black_king_plan.price}
            </span>
            /شهرياً
          </p>
        </div>

        <span className="text-start my-6 text-[#000000] text-xl  ">
          {black_king_plan.tagline}
        </span>
        <div className="w-full h-fit ">
          {black_king_plan.features.map((subitem) => (
            <div
              key={subitem?.text}
